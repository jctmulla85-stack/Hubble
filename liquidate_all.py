...
print("--- Queuing Full Portfolio Liquidation ---")
liquidated_count = 0
...
for p in positions:
    try:
        ...
        print(f" -> Queued Liquidation: {p.symbol} (Qty: {p.qty}, Value: ${float(p.market_value):.2f})")
        liquidated_count += 1
    except Exception as e:
        print(f" [ERROR] Failed to queue {p.symbol}: {e}")

print(f"\n[COMPLETE] Successfully queued market orders for all {liquidated_count} open positions.")
